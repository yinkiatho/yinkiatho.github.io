        ...
        >
          {props.work_experience.map((experience, index) => (
            <VerticalTimelineElement
              ...
              contentArrowStyle={{
                borderRight: "7px solid  var(--strong-color)",
              }}
              icon={
                <img
                  src={`assets/images/${experience.company_logo}.png`}
                  alt="company logos"
                  style={{ width: "110%", position: "relative", top: "-2.5px", left: "-2.4px" }}
                />
              }
              style={{paddingBottom: '20px', marginRight: '15px'}}
              key={index}
            >
...
const ProjectCards = (props) => {
    return (
      <>
        <Card
          className="project col-sm-12 col-md-4 col-lg-3"
        ...
        >
          ...
          <Card.Body className="d-flex flex-column">
            ...
            <Card.Text as="p">{props.description}</Card.Text>
            <div className="mt-auto">
              {Object.entries(props.links).map(([key, value]) => (
                <Button
                  className={`m-1 custom-primary`}
                  key={key}
...
const Resume = (props) => {
    ...
    React.useEffect(() => {
      const resizeObserver = new ResizeObserver((entries) => {
        for (const entry of entries) {
          const newWidth = entry.target.clientWidth * 0.95;

          // Check if there's a significant change in width
          ...
          <Accordion.Item eventKey="0">
            <Accordion.Header>
              ...
              <small className="text-muted">
                &nbsp;{props.accordion_text_muted}
              </small>
            </Accordion.Header>
            <Accordion.Body>
              <div className="d-grid gap-2 pb-2" id="resume_width_guide">
                <Button
                  className="m1 text-white"
                  variant="primary"
                  size="sm"
                  href={file}
                >
              ...
              >
                ...
                >
                  <Page width={width || undefined} scale={1.5} pageNumber={1} />
                </Document>
              </div>
            </Accordion.Body>
          </Accordion.Item>
...
const Hero = (props) => {
    return (
      <section
        ...
        style={{
          backgroundImage:
            "url(assets/images/new_wallpaper.png)",
        }}
      >
        <div className="container" data-aos="fade-right" data-aos-delay="100">
          ...
          <div className="social-links">
            {Object.entries(props.social).map(([link, icon]) => (
              <a aria-label={link} href={link} key={link}>
                <Icon icon={icon} />
              </a>
            ))}
          </div>
...
const Footer = () => {
    return (
      <footer id="footer" style={{borderRadius:"10px"}}>
        <div className="container">
          <h5>Yin Kiat Ho</h5>
          <p>Feel free to email me below for any opportunities. </p>
          <div className="social-links">
            <a aria-label="email" href="mailto:yinkiatho@gmail.com">
              <Icon icon="bx:bx-envelope" />
            </a>
          </div>
          <div>
            Made with <Icon icon="bx:bx-heart" color="var(--strong-color)" />{" "}
            and <Icon icon="bx:bx-code-alt" color="var(--strong-color)" /> using
            &nbsp;
            <a aria-label="React" href="https://reactjs.org/">
              <Icon icon="logos:react">React</Icon>
            </a>{" "}
            and &nbsp;
            <a aria-label="Bootstrap" href="https://getbootstrap.com/">
              <Icon icon="logos:bootstrap">Bootstrap</Icon>
            </a>
            .
          </div>
        </div>
    ...
    return (
      <section id="about">
        <div className="d-flex flex-wrap justify-content-center p-3 container">
          <div
            className="container d-flex flex-column align-items-center col-lg-6 col-sm-12 rounded"
            style={{ backgroundColor: "var(--off-bg-color)" }}
          >
            <div className="justify-content-center p-3 container">
              <img
                alt="profilepic"
            ...
            <div className="row justify-content-center align-self-start p-4">
              <h3>History</h3>
              <hr />
